...
glm.cv.ridge <- cv.glmnet(as.matrix(train_dat), y_train, alpha = 0)
glm.cv.lasso <- cv.glmnet(as.matrix(train_dat), y_train, alpha = 1)
glm.cv.net <- cv.glmnet(data.matrix(train_dat), y_train, alpha = 0.001)
...
glm.ridge <- glmnet(x = as.matrix(train_dat), y = y_train, alpha = 0, lambda = penalty.ridge )
glm.lasso <- glmnet(x = as.matrix(train_dat), y = y_train, alpha = 1, lambda = penalty.lasso)
glm.net <- glmnet(x = as.matrix(train_dat), y = y_train, alpha = 0.001, lambda = penalty.net)

y_pred.ridge <- as.numeric(predict(glm.ridge, as.matrix(train_dat)))
y_pred.lasso <- as.numeric(predict(glm.lasso, as.matrix(train_dat)))
y_pred.net <- as.numeric(predict(glm.net, as.matrix(train_dat)))


rmse_eval(Train$SalePrice, y_pred.ridge)
rmse_eval(Train$SalePrice, y_pred.lasso)
rmse_eval(Train$SalePrice, y_pred.net)



y_pred.ridge <- as.double(predict(glm.ridge, as.matrix(test_dat)))
y_pred.lasso <- as.double(predict(glm.lasso, as.matrix(test_dat)))
y_pred.net <- as.double(predict(glm.net, as.matrix(test_dat)))
...
x_train <- new.df[1:1456,]

x_test <- new.df[1457:nrow(df),]
